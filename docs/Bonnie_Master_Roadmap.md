...
>
> **Phase 0-A: Orchie UI + Mycelium UI** — Build the developer and user interface layer from the Claude Design reference components in `brand/`. Orchie chat interface, Mycelium control panel, sustain spec editor. This is your internal tooling environment — you build everything else with it.
>
> **Phase 0-A (parallel): Lore scaffold** — In parallel with the UI build, scaffold the Lore sustain structure: journal template (markdown-based, date-stamped entry format), docs index page, and about/contact placeholder pages. Lore must exist in skeleton form before any external communication begins — the journal is the signal that Sustena is alive.
>
> **Phase 0-B: WhatsApp Validation (Chama-first, Days 1–7)** — Runs in parallel with 0-A. Chama-first bot on WhatsApp to validate Orchie's transaction parsing with 50 real users. See `Sustena_XII_Master_Strategy.md §10.2` for full detail.
>
> **Phase 1: All Sustains launch together** — Homestead, Biashara, Mkulima, Colosso/Chama, Vyyb, and Lore launch as a unified first wave. Task list and epics for each sustain are in their respective sections below.
...
### Epic 1.7b — Lore Sustain: Public Knowledge and Narrative Layer

**Reference:** Master Strategy §3.1 (Lore as canonical sustain), Sustena XII `lore/` folder.

Lore is Sustena's public face. Without it, no external builder can onboard (no docs) and there is no public signal that Sustena exists (no journal). Lore ships in Phase 1 alongside all other sustains — not after.

- [ ] 🔴 **1.7b.1** Lore goes live — minimum viable public surface
  - [ ] `/journal` — at least 3 published journal entries (timeline of Sustena's discovery of a new paradigm). Format: date-stamped markdown, honest builder voice, no marketing. First entry should be live before any external outreach.
  - [ ] `/docs` — docs index live at `lore.dev/docs` (or `docs.sustena.io`). Minimum: Getting Started, the 7 Primitives, Operator Reference stub, Contributing to Mycelium. Wire to MkDocs build (per Epic 1.7.3).
  - [ ] `/about` — Sustena's mission, the founding trio, and the Sustena Loop. Plain language. No investor fluff.
  - [ ] `/contact` — email contact form or mailto link. Sufficient for Phase 1.
  - [ ] Whitepaper placeholder — a single page at `/whitepaper` that says the whitepaper is in progress, with a mailing list signup to be notified on release. The full whitepaper is drawn from `Sustena_XII_Master_Strategy.md` and ships in Phase 2.

- [ ] 🟡 **1.7b.2** `/q&a` — Frequently asked questions
  - [ ] Seed with 10–15 questions drawn from Substack reader responses and WhatsApp tester feedback
  - [ ] Update monthly as new questions emerge from community and investor conversations

**[TEST] Epic 1.7b — Milestone Criteria:**
- [ ] `lore.dev` (or subdomain) resolves and renders without errors
- [ ] At least 3 journal entries published, each 400+ words, with correct dates
- [ ] `/docs` index renders with at least 5 linked sections (Getting Started, Primitives, DSL, Operators, Contributing)
- [ ] `/about` and `/contact` pages render and are reachable from the home page nav
- [ ] Whitepaper placeholder page renders with mailing list capture
