...
>
> **Never label anything "not article-derived" without grepping the article first.** If it appears in an article, cite the §. A 2026-08-12 re-audit found this exact error in M-PAWA (the KES peg, royalty schedule and settlement stack were folded from a retired tracker and wrongly labelled) — corrected, and the rule written down so it does not recur.
...
*Every row here is **article-derived**, with its § cited. An earlier pass folded these from the retired `SUSTENA_WBD.md` and labelled them "not article-derived" — that was wrong: they are in the article bodies, and the label is corrected here. ADD-2 is the one genuine exception and says so on its own line.*
...
| ADD-2 | **Governance.** Two halves, and only one is article-derived. **(a) Article-derived — parameter governance:** every open parameter (κ_c, κ_s, φ_p, φ_r, fee rates, the issuance schedule, the treasury budget) is a **declared, revisable Sustain**; a parameter change is an **Enzyme admitted through `admit` with its approval token**, recorded in the ledger, auditable by replay — *"the on-chain-governance analog, expressed in Sustena's own construct"* (= **PAWA-11**). **(b) NOT in any article body — the RFC process:** earned contributor voting weight (merged contributions + Arena reputation + optionally staked pawa/Juul), propose → discuss → CouncilSession vote → finalize/anchor, quorum and **graduated rights** (proposer / reviewer / maintainer), community RFCs deciding `sustena_core`. This traces to `Tasks & Notes §4/§7/§9`, which Mycelium §VI·§X **cites but does not specify**. Per **CAP-17** the weight attaches to the **membership edge**, not the member | Pawa §6.5 (a) · *not article* (b) | ⏸️ ⬜ not started — (b)'s missing artefact is a governance doc, not code |
