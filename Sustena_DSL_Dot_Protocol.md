...
Sustena's dot protocol is not a programming language. It is a **naming convention + evaluation grammar** that sits on top of Python function calls and JSON state documents. Understanding it is the precondition for writing operators, declaring constraints, specifying sustain state, and building operatives.
...
**Step 3: User approves → Council votes → budget.allocate executes**
```
Constraint check:
