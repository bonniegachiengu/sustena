...
### Case-insensitive login fix + SMS auto-reader (M-Pesa + KCB) ✅ (1 Aug 2026)

**Case-insensitive email login**, diagnosed and fixed same-day as the Studio login bug report: `POST /login` compared `email == body.email` exact-case, so an account registered with any uppercase letter (autocapitalized once, years ago, or just typed that way) was unreachable by anyone typing it in lowercase — the normal way to type an email fresh on a new device with no autofill to copy the original casing from. Symptom: the same account, same password, worked on Android (autofill remembered the exact original casing) but failed on Studio desktop (typed fresh). Fixed by comparing `func.lower(email) == input.lower()` — no migration needed, works on every existing row regardless of its current stored case. `register()` now also normalizes to lowercase on insert and checks duplicates case-insensitively. 7 new tests. Verified live: registered a MixedCase throwaway account, logged in with the all-lowercase form (success), and confirmed a genuinely pre-existing account (from before this fix) logs in correctly with both its original casing and a different one.

**KCB SMS parsers added to `transducer.py`** alongside M-Pesa — `_parse_kcb` covers credit (mapped to `budget.record_income`), debit/transfer/fee (`parsed_unmapped`, pocket deliberately not guessed, same discipline as every M-Pesa outbound shape), and a new informational-only case for a standalone balance inquiry (no amount/direction — nothing moved, nothing to attach a pocket decision to). No `ingest_engine.py`/route changes needed — `source_id` was already fully generic (verified: zero "mpesa" hardcoding anywhere), so `"kcb"` is just another source_id string a client passes to the existing capture endpoint. **Honestly disclosed, not glossed over: the KCB wording is UNVERIFIED** — a best-effort structural guess at common Kenyan bank SMS phrasing, not checked against a real KCB message. Every capture group is named so retuning against 2–3 real (redacted) KCB SMS only ever needs to touch the regex bodies. 13 new tests exercise the parser mechanics against constructed samples — proving the code works, not that the wording matches reality.

**Native SMS auto-reader, Android/Orchie.** A custom Capacitor plugin (written from scratch rather than an unaudited third-party SMS-reading package, given the sensitivity of forwarding financial message content with a real auth token): `SmsReceiver.java` (a statically manifest-registered `BroadcastReceiver` for `SMS_RECEIVED`, survives the app not running) applies a strict privacy filter (sender must contain "MPESA" or "KCB" — `SmsSenderFilter`, the one file both the live and backfill paths share) and queues matching messages to a small local store; `SmsCapturePlugin.java` exposes `readInbox()` (one-time backfill) and `drainQueue()` to JS. Native code never touches the network — `smsCapture.js` is the only thing that calls `POST /api/v1/ingest/capture`, inheriting the real auth header via `api.post()`; capture is idempotent so retries are always safe. `OrchieShell.jsx`'s new `SmsCaptureCard` shows a plain-language rationale ("Orchie can automatically pick up M-Pesa and KCB transaction alerts... every other text is never touched") before ever triggering Android's real permission dialog — never on cold launch.

Honest scope: "real time" means captured instantly regardless of app state, but *delivered* on the next 20s local poll while foregrounded (or on resume) — not always-instant background push, which would need WorkManager-driven native networking; disclosed as later work, not built here. A real XML gotcha caught before packaging: `AndroidManifest.xml` comments using `--` as an em-dash (this codebase's normal style) aren't valid inside XML comments and failed the manifest merger — fixed by rewording, verified via `:app:processDebugMainManifest` directly rather than assumed from `:app:compileDebugJavaWithJavac` alone (that task had actually failed too, masked by piping through `tail` — a reminder to check real exit codes, not just tail'd output, before declaring a background build clean).

APK rebuilt: `versionCode` 3→4, `versionName` 1.0.2→1.1.0, same `applicationId` (installs as an update). Verified: `aapt dump permissions` confirms `READ_SMS`/`RECEIVE_SMS` declared and `SmsReceiver` registered with `android:permission=BROADCAST_SMS`; `apksigner verify --verbose` → Verifies (v2); `zipalign -c 4` → clean.

**Not verified from here, and can't be:** a real incoming SMS on an actual device, or a real KCB message correctly classifying end-to-end through the unverified transducer patterns. Bonnie confirms on his phone; **still need 2–3 real redacted KCB SMS samples from him to tune the regex wording**.

**Tests:** 20 new (7 case-insensitivity + 13 KCB transducer). **1764 backend tests pass**. Live on the public hostname (`git_commit` verified matching `HEAD`); new APK built, verified, delivered.
...
### In-field classify/confirm on Orchie + classification history + local notifications (1 Aug 2026)

**Design intent, stated firmly by Bonnie and held throughout:** the phone is the device in the field — a captured transaction must be classifiable/confirmable ON the phone, in the moment, never deferred to the laptop. Mycelium (the `/devui` cockpit) stays for heavy modelling; the classify/confirm loop belongs on Orchie.

**The wiring Bonnie flagged as a dependency turned out to already exist:** `unmapped_capture_classify` (declared on `homestead.json`'s `curated_widgets`, built in Slice 13) was already Unit-bound and already reading `IngestEngine.needs_attention()` directly on every `compose()` call — source-agnostic, so both `mpesa`- and `kcb`-sourced captures already reached the phone feed with zero code change needed there. What this pass actually adds is everything ELSE the brief asked for: the one-tap surface, classification history, and the local notification.

**1 — One-tap, not two:** `WidgetCard` (`OrchieShell.jsx`) previously gated `classify_card` behind a CLASSIFY button before opening the capture flow. Removed — a `classify_card` widget now opens `CaptureFlow` immediately on render, showing big-tap pocket options (or a pre-filled CONFIRM, see below) the instant the card appears. `curated_ui.py`'s `_render_unmapped_capture_classify` headline now states the real amount/merchant the transducer already extracted (`"Ksh 680 to NAIVAS SUPERMARKET — which pocket?"`) instead of a generic line, falling back to the old generic text only when the transducer genuinely couldn't recover either field.

**2 — Classification history ("purchase templates" — the loved VOS feature, carried over):** new `capture_classification_history` table in `ingest_engine.py` (`sustain_id, counterparty_key` primary key, recency-only model — the most recent confirm wins outright, disclosed as a real, simple limitation rather than building a weighted-vote system nothing asked for). `effect_capture.infer()` gained an optional `history` param: when text has zero pocket-name matches (the common unmapped case) and a history match exists for this counterparty *and the pocket still exists in live state* (a stale mapping to a renamed/deleted pocket is never followed), it pre-fills `pocket_name` and returns `status="ready"` directly — skipping the disambiguation TAP, never the CONFIRM tap. `InferenceResult` carries `from_history`/`history_use_count`/`description` so the frontend can label it ("pre-filled from how you classified this before") and a CHANGE button (sets `ignore_history=true` on the next `/orchie/capture/infer` call) lets a human override when the same merchant needs a different pocket this time. `capture_confirm` records the classification via `record_classification()` on a genuine success only, keyed by the `description` the frontend echoes back from the `infer()` result it just confirmed — best-effort, wrapped so a template-memory write failure can never be mistaken for the real transaction (already committed) having failed.

**3 — Local notification when a capture needs a decision:** `@capacitor/local-notifications` (official plugin, not hand-rolled — unlike the SMS-reading plugin, a purely local, on-device notification never leaves the phone, so it doesn't carry the same exfiltration-risk class that justified writing `SmsCapturePlugin` from scratch). `smsCapture.js`'s `forwardMessages()` now reads the real capture response (`POST /api/v1/ingest/capture`'s `_ok()` envelope — the actual outcome lives at `resp.data`, not `resp` itself, a real shape distinction caught before writing this) and fires a local notification titled "Orchie needs a decision" with the real amount/merchant in the body, but only when `is_duplicate !== true` (a backfill re-forwarding an already-seen unresolved item must never spam a fresh nudge for something the person already had a chance to act on). Permission (`POST_NOTIFICATIONS`, Android 13+) requested silently, best-effort, right after SMS capture syncs successfully — no separate rationale screen, since a local notification isn't the same "dangerous permission" class the existing SMS-read rationale card exists for; the existing card's copy gained one sentence disclosing the notification behavior for transparency.

**Amber-generality / scope discipline held:** none of `effect_capture.py`'s or `ingest_engine.py`'s new code references "homestead"/"habitat" — the history table is keyed purely by `sustain_id` + a normalised counterparty string, generic to any sustain declaring `finances.pockets`.

**Verification:** backend fully covered by 4 new end-to-end HTTP tests in `test_orchie_capture_routes.py` (first capture from a merchant still asks; second capture from the same merchant pre-fills with `from_history=True`; `ignore_history` forces the normal ask back open; history never leaks across a different sustain) plus 8 new `ingest_engine.py` unit tests (key normalisation, use_count increment, per-sustain scoping, safe no-ops) and 12 new `effect_capture.py` unit tests (`resolve_description`, history pre-fill, stale-pocket fallback, text-match-beats-history precedence). **1817 backend tests pass.** A live browser walkthrough of the rendered `/orchie` page was attempted but blocked by a pre-existing, unrelated local-dev-only quirk: `vite.config.ts`'s dev proxy forwards `/orchie*` to the backend API (since `/orchie` is also a real API prefix — `/orchie/message`, `/orchie/compose`, etc.), which collides with the client-side `/orchie` page route on a fresh top-level navigation in the Vite dev server specifically (the deployed production process has no such split — one process serves both, and the existing static-file-vs-SPA-fallback routing fix from the PWA slice already handles this correctly there). Not a regression from this pass; flagged as a known dev-tooling rough edge, not fixed here since `vite.config.ts`'s proxy rules are shared, unrelated infrastructure.

**APK:** `versionCode` 6→7, `versionName` 1.1.2→1.2.0 (a real feature bump, not a patch), same `applicationId` (installs as an update). New permissions from the local-notifications plugin's own manifest merge: `POST_NOTIFICATIONS`, `RECEIVE_BOOT_COMPLETED`, `WAKE_LOCK` — confirmed via `aapt dump permissions` alongside the pre-existing `READ_SMS`/`RECEIVE_SMS`. `apksigner verify --verbose` → Verifies (v2); `zipalign -c 4` → clean.

---

### Real-time native classify notification + Sustena's own palette (1 Aug 2026)

Two refinements to the in-field classify/confirm work above, both requested explicitly by Bonnie.

**1 — Real-time, not "next time he opens the app."** The prior pass's notification only fired from JS (`smsCapture.js`'s `forwardMessages()`), reached only via the 20s `startLivePolling()` interval or the next `backfillInbox()` — both require the WebView's JS to actually be running, which Android is free to suspend the moment the app leaves the foreground. Honest fix, not a shorter poll interval: SmsReceiver (the manifest-registered `BroadcastReceiver`, already fires regardless of app state) now ALSO enqueues a `WorkManager` job (`IngestWorker.java`, new) directly, in addition to its existing `SmsQueueStore` enqueue (kept, unchanged, as the fallback belt-and-suspenders path).

- `IngestWorker` is the one and only place in this app where NATIVE code makes a network call — a deliberate, disclosed exception to "native never talks to the network" (that rule was about privacy/exfiltration risk for the SMS-reading path specifically; this is the app's own already-authenticated capture call, and "immediate regardless of app state" cannot honestly be built any other way on Android). It reads a cached token/sustain_id/api_base from a new `SmsAuthStore` (SharedPreferences — JS pushes fresh values via `SmsCapturePlugin.setAuthContext()` whenever the token or the selected sustain changes, since native code can't read the WebView's localStorage), POSTs directly to `/api/v1/ingest/capture` via plain `HttpURLConnection` (no new HTTP dependency), and — on a genuine, non-duplicate `needs_attention` result — builds and shows the "Orchie needs a decision" notification itself via `NotificationCompat`, entirely without JS.
- Missing auth context (never signed in on this device, or JS hasn't synced yet) is a silent, safe no-op — `SmsQueueStore`'s existing enqueue (still happens first, unconditionally) is the fallback the JS poll/backfill path already handles. Both paths safely overlap because capture is idempotent (`dedup_key`) — whichever one wins the race, the other's later attempt just comes back `is_duplicate: true` and is correctly skipped by the existing notify-guard, so there is no double-notification risk.
- A real minSdk bug caught before it could ship: `java.time.Instant` needs API 26+ (or desugaring, not configured in this project's build) but `minSdk` is 24 — using it would have crashed `IngestWorker` on Android 7.0/7.1 devices. Replaced with a `SimpleDateFormat`-based ISO-8601 formatter (API 1+) producing the identical shape JS's own `toISOString()` already sends.
- Tapping the notification deep-links straight to that item's classify card: the `PendingIntent` targets `MainActivity` with `sustena_classify_sustain_id`/`sustena_classify_message_id` extras; `MainActivity.onCreate`/`onNewIntent` (the app's `singleTask` launch mode routes a repeat tap through `onNewIntent`, not a fresh `onCreate`) stash them via `SmsAuthStore.setPendingClassifyTarget()`; `OrchieShell.jsx` consumes them once (`consumePendingClassifyTarget()`, via a new `SmsCapturePlugin.consumePendingClassifyTarget()` method) on mount and on `visibilitychange`, switches to the target sustain if needed, and renders a `CaptureFlow` bound directly to that `message_id` in a highlighted "FROM NOTIFICATION" section above the normal ranked feed — independent of whatever `compose()` would otherwise have surfaced first.
- New `androidx.work:work-runtime:2.9.1` dependency; `SmsReceiver` enqueues the job with `OutOfQuotaPolicy.RUN_AS_NON_EXPEDITED_WORK_REQUEST` (run as soon as possible, degrade to normal background work rather than fail if there's no expedited-job quota — no extra foreground-service permission needed for that fallback path itself, though WorkManager's own manifest merge does pull in `ACCESS_NETWORK_STATE`/`FOREGROUND_SERVICE`, confirmed via `aapt dump permissions`).

**2 — Sustena's own palette, not VOS's.** Bonnie was explicit: VOS used safety-orange for outgoing / magenta for incoming, and that convention must not leak into Sustena. A `DirectionBadge` (`OrchieShell.jsx`, new) renders a plain `+`/`−` before a classify card's headline, colored using tokens `globals.css` already assigns real meaning to elsewhere in this exact file — `--teal` for "+" (received, same color CONFIRM/committed-success already use) and `--amber` for "−" (sent, Sustena's existing default/primary accent, not an alarm color). No new hues introduced anywhere in this pass.

**Tests:** no new backend tests this pass (no `.py` files touched — pure native Android + frontend JS/JSX). Full backend suite re-confirmed green regardless (**1817 pass**, unchanged) per this project's own "must pass clean before every commit" discipline. Frontend builds clean in both modes (`npm run build`, `npm run build:capacitor`).

**APK:** `versionCode` 7→8, `versionName` 1.2.0→1.3.0, same `applicationId` (installs as an update). New permissions from `androidx.work`'s own manifest merge: `ACCESS_NETWORK_STATE`, `FOREGROUND_SERVICE` — confirmed via `aapt dump permissions` alongside everything already declared. `apksigner verify --verbose` → Verifies (v2); `zipalign -c 4` → clean. Merged manifest independently checked to confirm `SmsReceiver`'s existing registration and `WorkManager`'s self-initializing `androidx.startup` provider both landed correctly.

**Not independently verified from here, disclosed honestly:** a real incoming SMS triggering the full native chain (BroadcastReceiver → WorkManager → HTTP POST → notification → tap → deep-link) end-to-end on an actual device — this requires a real phone receiving a real SMS, which isn't available in this environment. Every individual piece was verified as far as this environment allows: the Java compiles and packages cleanly, the manifest merge is correct, the minSdk-24 timestamp fix was verified against the actual API level constraint, and the identical JS-side capture→classify→confirm→history flow (which `IngestWorker`'s POST target is byte-identical to) was already verified live end-to-end against the public API in the prior pass. Bonnie confirms the real-device, real-SMS path.
...
### Real screenshots resolve the source mystery: source-strict parsing, real card formats, expanded secret filter (2 Aug 2026)

Bonnie's KCB screenshots gave real sender/text evidence for the pending FIX 2 investigation, plus three real card-transaction texts and three real secret shapes the filter missed. Four things landed.

**FIX 2 — source mis-tagging, root-caused as far as static analysis can go, and structurally closed regardless:** exhaustive re-audit of every source-classification call site (`smsCapture.js`'s `classifySource()`, `IngestWorker.java`'s `classifySource()`, `SmsSenderFilter.isKnownFinancialSender()`) confirmed — again — that none of them ever inspect message BODY content; all three are purely sender-string-based, and the two `classifySource()` copies are byte-identical. No code path was found that infers source from text. What the audit found instead, and fixed, is a more fundamental structural gap: **`transducer.py`'s `parse_message()` never actually used `source_id` to gate which parser set ran** — it always tried every `mpesa_*` regex first, for every message, regardless of what source_id the client tagged it with. `source_id` was pure display metadata. This is real, independent of whatever caused the specific mis-tag Bonnie saw: nothing was structurally preventing a broadly-written future `mpesa_*` regex from accidentally matching KCB-worded text (several KCB messages legitimately say "M-PESA ref ..." in their own wording), silently overriding what the sender already established. Fixed: new `_PARSERS_BY_SOURCE` dict (`{"mpesa": [_parse_mpesa], "kcb": [_parse_kcb]}`); `parse_message()` gained an optional `source_id` param that, when it names a known source, restricts parsing to ONLY that source's own parser set — body content can never again promote a message out of its own sender's set. `ingest_engine.py._process()` now passes the real `source_id` through. Unknown/omitted `source_id` falls back to the original try-everything behaviour (backward compatible with every existing call site/test). As defense-in-depth (not because a literal bug was found there), `classifySource()`'s check order was also flipped to KCB-before-MPESA in both copies, so sender-string ambiguity can never silently favor "mpesa" either.

**FIX 3 — real card formats, tested directly: already working; a real direction bug found instead.** Tested the exact real card texts (`KES 429.00 ... GOOGLE *Spotify Music ... Avail balance KES 59,055.00 Enquiries call ...Pata extra cash...`, the `g.co/helppay#` sample) directly against the existing `_KCB_CARD_RE` — both matched correctly already; "Avail balance" (no "able"), lowercase "12:25pm", and the "Pata extra cash..." trailer were all already handled by the existing regex's own tolerances. Replaced the two existing `KCB_CARD_KES`/`KCB_CARD_USD` test fixtures (which used a speculative, never-actually-real "...as at 01/08/2026 12:25pm" trailer) with these authoritative real texts; added a third fixture (`KCB_CARD_HELPPAY`) as a new, separate test. The USD sample was given truncated in the screenshot excerpt — reconstructed following the same real trailer template the two KES samples confirmed, disclosed as such rather than presented as independently verified.

What WAS genuinely broken, found by testing the real paybill text (`"Ksh 40000.00 sent to KCB Pay Bill 522522 for account 135***5140 BONVENTURE NGUGI MAINA has been received on 01/08/2026 at 12:21 PM. M-PESA ref UH1B91GYNW..."`): `kcb_mpesa_paybill`/`kcb_mpesa_account` (built two sessions ago) had the DIRECTION BACKWARDS. Despite "sent to KCB Pay Bill... has been received" reading like an outbound payment, this is money being CREDITED INTO the account the SMS is about — "Pay Bill 522522" is KCB's own deposit paybill, and "for account ... NAME has been received" names the recipient (the account holder), not the sender. Both handlers were `parsed_unmapped, direction="sent"`; fixed to `direction="received"` and — matching the same certainty class as the sibling `kcb_mpesa_received` parser — mapped to `budget.record_income` instead of left needing a manual pocket decision that was never actually ambiguous. Verified directly: `source=kcb` on the real paybill text now returns `status=mapped, operator=budget.record_income, direction=received`; the same text with a deliberately WRONG `source=mpesa` tag correctly returns `unparsed` (proving this exact paybill shape genuinely isn't matched by any of the 5 `mpesa_*` regexes — confirming it as real KCB-shaped content, and reproducing the exact "no registered parser" symptom Bonnie originally saw under a hypothetically-mistagged capture).

**FIX 4 — secret filter expanded, five new patterns:** `tan\s+code`, `activation\s+code`, `secret\s+pin`, `\bpin\s+is\b`, `code\s+is\s+valid` (all case-insensitive), added to BOTH `transducer.py`'s `_SENSITIVE_SECRET_PATTERNS` (server-side defense-in-depth) and `SmsSecretFilter.java`'s identical Java list (Android primary defense — these should never leave the device at all). Verified against three real shapes from Bonnie's actual KCB thread (code digits replaced with placeholders in every committed fixture/test, same discipline already used for real names elsewhere in this file — these are live-ish secrets): a TAN code, an "Activation code: ... valid for only 90s" message, and a "KCB card secret PIN is ..." message — all three now `status="rejected"`, none leak `parsed_fields`. False-positive check re-run against every real transaction fixture already in the test module (21 of them) to confirm none of the five new patterns misfire on genuine transaction vocabulary.

**Also, per the brief:** informational KCB branch-visit notices ("Dear customer, thank you for visiting..." style, no amount/transaction) were already correctly handled as informational rather than a spend — no change needed, `_KCB_BALANCE_RE`/loan-status parsers already establish that "recognised but nothing to map" pattern and a bare branch-visit notice with no financial figures would already fall to a safe, honest `unparsed` rather than being misread as a transaction.

**Tests:** 20 new/replaced (`TestSourceStrictParsing` — 7, `TestExpandedSecretPatterns` — 5, `test_helppay_card_transaction...` — 1, 2 existing `kcb_mpesa_paybill`/`account` tests updated for the direction fix, plus the two card fixtures replaced with authoritative real text). **1845 backend tests pass.** Frontend builds clean in both modes (only `smsCapture.js`'s comment/reorder changed — no behavior-affecting JS beyond the defense-in-depth reorder).

**APK:** `versionCode` 9→10, `versionName` 1.3.1→1.3.2, same `applicationId` — ships the `SmsSecretFilter.java` pattern expansion + `IngestWorker.java`'s classifySource reorder to the native app. `apksigner verify --verbose` → Verifies (v2); `zipalign -c 4` → clean.

**Housekeeping caught while committing this pass:** the previous pass's version bump (`versionCode` 8→9, `versionName` 1.3.1) had been made on disk and shipped in the APK sent to Bonnie, but `apps/web/android/app/build.gradle` was never included in that commit — a real staging oversight, not a functional bug (the APK itself was correctly versioned). Folded into this commit rather than left dangling.

---

### Classify card: raw text, real degrees of freedom, and confirming the audit substrate (2 Aug 2026)

Bonnie's follow-up on the classify-card work, explicitly framed as human-in-the-loop correction: (1) show the exact raw SMS, (2) let a wrong or missing parse be FULLY corrected — amount, direction, description, pocket — not just "pick a pocket", (3) confirm raw messages are retained and queryable, since an audit/correction-history/parser-training surface is coming next. Explicitly NOT this pass: that audit UI or any learning loop.

**1 — Raw text on the card: already shipped, verified still present.** `curated_ui.py`'s `_render_unmapped_capture_classify` already includes `raw_payload` in the widget's data (since Slice 14), and `WidgetCard`'s classify_card block already renders it in a small monospace context box (added in the dead-end fix pass). No new work needed — confirmed intact, not re-built.

**2 — Real degrees of freedom, the actual new work.** The `unmapped_capture_classify` widget's declared `emits` (`homestead.json`) gained `budget.record_income` alongside `budget.spend`/`budget.allocate` — previously this widget had NO way to route to income at all, a real, disclosed scope boundary from the earlier dead-end-fix pass, now closed because Bonnie explicitly asked for it.

- **Direction toggle, backend half:** `effect_capture.infer()` gained income verb hints (`received`/`credited`/`deposited`/`got paid`/`income` → `record_income`, alongside the existing spend/allocate verbs) and a `direction=="received"` narrowing symmetric to the existing `direction=="sent"` one. More importantly, a REAL bug in the existing flow was found and fixed: pocket resolution used to run unconditionally, even when the eventually-chosen operator (`budget.record_income`, which has NO `pocket_name` param at all) would never use it — meaning a genuinely-received message was previously forced through an irrelevant pocket question before the person ever got a chance to say "this was money IN." Fixed with a `pocket_relevant = any("pocket_name" in _param_names(op) for op in ops)` guard around the whole pocket-resolution block — an explicit `known.operator="budget.record_income"` (what the frontend's MONEY IN toggle sends) now skips pocket entirely, at every call site, not just the final build step. The genuinely-ambiguous case (no hint at all) still surfaces "money received" as a real third tap option in the existing operator disambiguation, correctly humanized (`_OPERATOR_LABELS["budget.record_income"] = "money received"`).
- **Full editable review, frontend half:** `CaptureFlow`'s old two-line "ready" confirmation was replaced with a new `ReadyReview` component showing FOUR genuinely editable/correctable things before the single CONFIRM tap: a MONEY IN / MONEY OUT toggle (backed by the new `setDirection()` — sets or clears `known.operator`), an editable amount field (pre-filled with whatever was inferred, corrects via the same `known`-merge-and-re-infer contract every other field already uses), an editable description/merchant field (same mechanism), and the resolved pocket with a generalized CHANGE button (now separate from the history-specific CHANGE — `changePocketFromHistory` vs the new `changePocket`, both clearing the relevant fact and letting normal resolution/asking resume) that only renders when the resolved operator actually has a pocket_name param. Every edit re-runs the real `/orchie/capture/infer` call — what's shown is always exactly what CONFIRM would send, never a local-only preview that could drift.

**3 — Raw message retention + queryability: confirmed, and one real gap closed.** `ingest_messages.raw_payload` is `NOT NULL`, has no deletion path anywhere in the codebase (grepped for `DELETE FROM ingest_messages` — zero matches), and is already queryable via `GET /api/v1/ingest/messages?sustain_id=` (list, with `status`/`limit` filters) and `GET /api/v1/ingest/messages/{id}` (single) — both real, ownership-checked routes, confirmed with a new end-to-end test capturing a message and reading `raw_payload` back byte-for-byte through both routes. One real gap found and closed while confirming this: `GET /messages` previously made `sustain_id` OPTIONAL, and omitting it silently returned **every sustain's messages with no cross-user filtering** ("an admin-style view", per its own old docstring) — confirmed unused by grepping the frontend (zero callers exist yet, since the audit UI hasn't been built). Since this route is about to become load-bearing for exactly the audit feature Bonnie described, tightened it now while it's free: `sustain_id` is required (422 without it), always ownership-checked. `rejected` (OTP-carrying) captures remain correctly and deliberately never stored at all — that's the one, intentional exception to "every raw message is retained," unchanged from the original security design.

**Tests:** 14 new (`test_effect_capture.py`: `TestDirectionCorrection` — 7, covering the pocket-never-asked-before-income-resolves property directly; `test_ingest_routes.py`: `sustain_id`-required + raw-payload-retrievable — 2; `test_orchie_capture_routes.py`: `TestFullCorrectionFreedom` — 5, full HTTP round trips for direction-to-income, amount correction, description correction, direction-back-to-out, and raw text retrieval). **1859 backend tests pass.** Frontend builds clean in both modes.

**APK:** `versionCode` 10→11, `versionName` 1.3.2→1.4.0 (a real feature addition, not a patch) — ships the new `ReadyReview`/direction-toggle frontend code (`homestead.json`'s widget-emits change is bundled server-side, not the APK). `apksigner verify --verbose` → Verifies (v2); `zipalign -c 4` → clean.
...
### Action feedback states + real M-Pesa parser gaps + add-pocket operator (2 Aug 2026)

Four things folded into one round, per Bonnie's own grouping.

**A — Action feedback: found and fixed the real root cause, not just added a spinner.** Bonnie's report ("tap CONFIRM, the card just sits there") traced to a genuine React bug, not a missing feature: `WidgetCard`'s `onCommitted` handler called `setCapturing(false)` in the SAME synchronous tick as `CaptureFlow`'s own `setPhase('committed')` — React 18 batches both updates into one re-render, so `CaptureFlow` unmounted before its own success message ever painted. The card didn't fail to show feedback; it showed feedback for zero frames. Fixed: `onCommitted` now only triggers the background data refresh; collapsing the card is entirely `CaptureFlow`'s own decision (its CLOSE button, or a new auto-dismiss timer). Built out from there:
- **In-progress:** CONFIRM tap → `recording…` with a pulsing amber dot (reusing globals.css's existing `.pulse` keyframe, not a new spinner asset).
- **Success:** `✓ recorded — Ksh {amount} to {pocket}` (or `— Ksh {amount} received` for income) — a real plain-language summary captured from the pre-confirm `ready` payload (needed since the `/confirm` response overwrites `payload` with a differently-shaped body that no longer carries `operator`/`params` under those names). Auto-dismisses after 2.6s (CLOSE still available for an immediate manual dismiss) so the card is never left ambiguously sitting there.
- **Refusal:** `✗ not recorded — {reason}`, stays visible (no auto-dismiss), now with a real RETRY button (re-runs `infer()` with the same known facts, landing back at an editable `ready` review rather than a dead end).
- A second, smaller real bug fixed while auditing this: `view.selected.map(w => <WidgetCard key={w.id}>)` used `w.id`, which is always the literal `"unmapped_capture_classify"` string for EVERY classify card regardless of which message it represents (`compose()` never qualifies it) — a real key-collision risk if two messages ever need attention at once. Now keys on `message_id` when present.

**B/D — Real M-Pesa parser gaps, from Bonnie's own screenshots.** Two genuinely new Safaricom SMS shapes, tested against the exact real text before writing any regex:
- `mpesa_paybill_sent` — `"Ksh1,500.00 sent to SAFARICOMHOME for account 11619547 ... New M-PESA balance is Ksh12,154.47. Transaction cost, Ksh0.00."` The existing `mpesa_paybill` parser only recognised "**paid** to X for account Y"; Safaricom genuinely also uses "**sent** to X for account Y" for the same paybill-payment shape — textually distinct from every existing pattern (checked before adding), so no ordering risk.
- `mpesa_airtime` — `"You bought Ksh50.00 of airtime on 1/8/26 at 11:15 PM. New M-PESA balance is Ksh13,654.47..."` — an entirely different sentence shape with no counterparty. The real sample also has lowercase `"confirmed."` (every other real M-Pesa sample in this codebase capitalises it) — `re.IGNORECASE` added specifically because of that *observed* variation, not applied speculatively.

Both extract `transaction_cost` (a new field, from `"Transaction cost, Ksh0.00"`) alongside the existing amount/counterparty/balance fields. The already-existing 5 patterns (received/paybill "paid to"/buygoods/sent-to-person/withdraw) were re-verified as still correct and untouched — Bonnie's "add representative formats" ask for those is already satisfied by the original set. No M-Pesa balance-inquiry pattern was added — no real sample exists for one (KCB already has its own), and this project's own hard-learned lesson from the earlier speculative-KCB-pattern episode is not to guess a format without real text.

**The allocate/spend sanity check, confirmed:** both new shapes carry `direction: "sent"`, which `effect_capture.infer()`'s existing narrowing (`direction=="sent" → only budget.spend remains a candidate`) already uses — verified directly, running the REAL text through the full `parse_message() → infer()` pipeline with `budget.record_income` also a live candidate: both resolve to `operator: "budget.spend"`, never `budget.allocate`. This was already correct by construction once the parser gap was closed; the fix wasn't in the narrowing logic, it was in getting `direction: "sent"` onto the message at all (an `unparsed` message carries no `parsed_fields` whatsoever, so the narrowing never got a chance to run before these parsers existed).

**C — "+ NEW" pocket, via a real gated operator (new: `budget.add_pocket`).** No existing operator could create a bare, empty pocket — `budget.allocate` always requires `amount > 0` (a hard constraint) and always debits liquid balance, so it can't be used to just NAME a pocket before deciding how much goes into it. Added `budget.add_pocket(pocket_name, limit=0.0)`: creates `finances.pockets.<name>` with `allocated=spent=0`, no money movement, fires `event.finances.pocket_created`, refuses honestly (not a silent no-op) if the pocket already exists. Registered in `OPERATOR_REGISTRY` and `ALL_KNOWN_OPERATORS` per the standing rule.

A real constraint caught by testing, not by inspection: `StateAccessor`'s own path parser only accepts `\w` segments (`state.py`'s `_SEGMENT_RE = r"(\w+)(?:\[(\d+)\])?"`) — every pocket name in this codebase had always incidentally been a single word ("food", "WiFi") until now, since nothing before this let a person type an arbitrary name. A raw `"holiday fund"` (with a space) raised an unhandled `StatePathError`, not a clean refusal — caught by the FIRST test written against it. Fixed with `_normalize_pocket_name()`: collapses any run of non-word characters to a single underscore (`"holiday fund"` → `"holiday_fund"`, `"  Car-Repair!!  "` → `"Car_Repair"`) rather than bluntly rejecting a perfectly reasonable typed name; refuses only if the result is empty. The operator's real, final `pocket` name is what's returned and what the frontend continues the flow with.

**Frontend — `PocketPicker` (new), replacing the plain tap-grid for the `pocket_name` disambiguation specifically:** every existing real pocket still renders as a tap option; a dashed `+ NEW` tile opens an inline name field; CREATE calls `budget.add_pocket` through the same `/orchie/capture/confirm` route every other write already uses (no new backend route needed — `message_id` is optional there), shows the create/success/error feedback states (per Part A's discipline), and on success hands the REAL server-normalised name to the same `answer()` continuation the tap-options use — the newly created pocket immediately becomes the one this transaction files into, and appears in the grid the next time it's shown.

**Tests:** 23 new (`test_transducer.py`: `TestNewMpesaShapes` — 5; `test_effect_capture.py`: 2 new sanity-check tests running the real text through the full `parse_message → infer` pipeline; `test_budget_operators.py`: `TestBudgetAddPocket` — 7, including the space-normalisation bug found live; `test_operator_registry.py`/registry-list updates). **1882 backend tests pass.** No new frontend tests (verified via build + the same live-API-call discipline used throughout this project for UI work). Frontend builds clean in both modes.

**APK:** `versionCode` 12→13, `versionName` 1.4.1→1.5.0 (real feature additions across all four parts) — same `applicationId`. `apksigner verify --verbose` → Verifies (v2); `zipalign -c 4` → clean.
