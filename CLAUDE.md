...
> and how Bonnie works. Everything here is current as of 27 Jul 2026 (updated through Slice 7 — Coordination & roll-up).
...
**Design fork raised, not guessed — confirmed with Bonnie before writing the gate code:** when a child operator's own gate passes but the resulting roll-up would push a parent-level aggregate invariant out of its viable region, who has authority? Two real options existed — parent-vetoes (refuse the child's own transaction, requiring the engine to reach into a different sustain's invariants at commit time, and a real distributed-transaction question with no clean answer in this single-connection-per-engine architecture) vs. parent-observes-never-vetoes (the child commits normally; the violation is surfaced for display, never blocks). **Bonnie chose parent-observes-never-vetoes** — matching the Multiparty article's own CRDT model (the group value is a *merge* over member entries, never an overwrite; "never erase the node").
...
**Move 4 — the gate governs display, never blocks (`_compile_spec_invariants`'s `enforced` flag):** an invariant is compiled exactly as before, but is now additionally checked (via a small AST walker, `_referenced_root_names`, reusing `predicates.py`'s existing typed node classes — no new parser) for whether it references a dimension declared in the sustain's own `aggregates` list. If so, `enforced=False` is stamped on it. `_check_enforcement_gate` (used by both `execute_operator` and the new `commit_external_mutation(check_gate=True)`) skips any `enforced=False` invariant — on ANY sustain's own operator calls, not just a child's. This is deliberate: an aggregate can't change because of what the sustain *itself* does, so gating on it there would just be a permanent, confusing block unrelated to the operator being run. `evaluate_constraints()` (the Monitor's constraint-health/needs-attention display path) now reads `_state_with_aggregates()` instead of raw state, so an aggregate-referencing invariant still shows a real pass/fail — surfaced, never enforced. Proven end-to-end with a synthetic parent/child pair: pushed a child negative (its own gate allows it — no invariant of its own), confirmed the child's operator still succeeds, confirmed the PARENT's own unrelated operator also still succeeds despite the now-violated aggregate invariant, and confirmed the violation shows up as `status: "fail"` in `evaluate_constraints()`.
...
- **Structured JSON-valued params for votes/allocations** — checked, found not applicable: `sustains.py`'s bodies (`VoteBody`, `ExecuteOperatorBody`) are already real JSON request bodies, not the free-text Console parser that broke on `edit.state_patch`'s list-shaped `patch` param in Slice 6. That console-parser gap is unrelated to this route file and stays exactly where it was left — its own flagged follow-up task, not touched here.
...
**Tests:** `tests/test_composition.py` (38 — link/list/unlink/get_parent, provision idempotency, roll-up sum/avg/min/max/count, honest exclusion of a missing child, the full parent-observes-never-vetoes proof, `commit_external_mutation`'s `check_gate` opt-in) + `tests/test_composition_routes.py` (11 — the new devui.py routes, `/devui/state` carrying a real rollup) + 3 new tests in `test_api.py` (the vote route wires `check_gate=True`, a gate-refused vote returns 422 with the reason, `sustains.get_engine()` identity-matches the real shared singleton). **1483 tests pass** (1431 + 52). Frontend builds clean (`npm run build`). Live verification across direct-engine, real HTTP API, and real browser UI, against Bonnie's actual homestead — confirmed via `/health` throughout, VOS/tunnel/keepalive untouched.
